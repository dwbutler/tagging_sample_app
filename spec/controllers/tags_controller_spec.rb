...
RSpec.describe TagsController, :type => :controller do
  describe "POST #create" do
    it "creates a new entity" do
      post :create, params: { entity_type: 'Article', entity_id: 10 }
      expect(response).to be_success
      expect(Article.find(10)).to_not be_nil
    end

    it "creates tags" do
      post :create, params: { entity_type: 'Article', entity_id: 1, tags: %w(a b c) }
      expect(response).to be_success
    ...
    it "replaces tags on an existing entity" do
      ...
      article.tag!("a", "b")
      post :create, params: { entity_type: 'Article', entity_id: article.id, tags: %w(foo bar) }
      expect(response).to be_success
  ...
  describe "GET #show" do
    ...
    it "returns Not Found for entity that does not exist" do
      get :show, params: { entity_type: 'Article', entity_id: -1 }
  ...
  describe "DELETE #destroy" do
    it "deletes an existing entity and its tags" do
      article = Article.create!
      article.tag!(%w(a b))
      delete :destroy, params: { entity_type: 'Article', entity_id: article.id }
    ...
    it "returns Not Found if the entity does not exist" do
      delete :destroy, params: { entity_type: 'Article', entity_id: -1 }
      expect(response).to be_not_found
    end
  end
end
